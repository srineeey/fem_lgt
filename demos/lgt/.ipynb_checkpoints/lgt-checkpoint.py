...
def set_gfU_const(gfU, qtheta=0., theta=0., phi=0., glob_or=True):
    ...
    set_gfU_qconst(gfU, q, True)
        
    #return gfU

# Set all link values constant
def set_gfU_qconst(gfU, q, glob_or=True):
...
def random_links(gfU, qthetamax=2*pi, thetamax=pi, phimax=2*pi):
    mesh = gfU.space.mesh
    qthetas = qthetamax*np.random.rand(mesh.nedge)
    thetas = thetamax*np.random.rand(mesh.nedge)
    phis = phimax*np.random.rand(mesh.nedge)
